...
OLD_MOBILE_MAIN = "1c49a6f7fd236c30e0539b510d6818470a356273"
OLD_BACKEND_MAIN = "2c683c95274409aa5958033e96cb8acf67ca8b56"
...
def replace_after(
    text: str, anchor: str, old: str, new: str, label: str
) -> str:
    ...
    if match_index < 0:
        raise SystemExit(f"{label}: marker missing after anchor")
...
def update_baseline(text: str, label: str) -> str:
    text = replace_once(
        text,
        f"- mobile `main`: `{OLD_MOBILE_MAIN}`;",
        f"- mobile `main`: `{MOBILE_MAIN}`;",
        f"{label} mobile baseline",
    )
    text = replace_once(
        text,
        f"- backend `main`: `{OLD_BACKEND_MAIN}`;",
    ...
    return replace_first(text, marker, addition, f"{label} baseline evidence")


ROADMAP_PR111 = f"""Backend PR #111, exact green head `{PR111_HEAD}`, merge `{BACKEND_MAIN}`:
...
ROADMAP_NEXT = """Remaining P5 boundary:
...
PLAN_PR111 = f"""Backend PR #111, exact green head `{PR111_HEAD}`, merge `{BACKEND_MAIN}`, completed the production-shaped Resend runtime boundary:
...
PLAN_NEXT = """Complete the Resend backend composition and readiness boundary next without activating delivery:
...
provider_evidence = f"- backend PR #110 merge: `{PR110_MERGE}`;"
provider_evidence_new = (
    provider_evidence
    + f"\n- backend PR #111 exact green head: `{PR111_HEAD}`;"
    + f"\n- backend PR #111 merge: `{BACKEND_MAIN}`;"
)
provider = replace_after(
    provider,
    "## Phase P5 — password-reset product and delivery readiness",
    provider_evidence,
    provider_evidence_new,
    "provider P5 evidence",
)
provider = replace_once(
    provider,
