...
describe('repository button control-copy audit', () => {
  it('keeps primary button labels behind localization boundaries', () => {
    const violations = presentationRoots.flatMap((root) =>
      collectPresentationFiles(resolve(projectRoot, root)).flatMap((path) => {
        const source = readFileSync(path, 'utf8');
        const quotedViolations = quotedLabelPatterns.flatMap((pattern) => {
          pattern.lastIndex = 0;
          return [...source.matchAll(pattern)].map(
            (match) =>
              `${relative(projectRoot, path)}:${lineNumberAt(source, match.index ?? 0)} static button label: ${match[2]}`,
          );
        });
        const templateViolations = templateLabelPatterns.flatMap((pattern) => {
          pattern.lastIndex = 0;
          return [...source.matchAll(pattern)].flatMap((match) => {
            const literal = match[1] ?? '';
            return hasStaticWords(literal)
              ? [
                  `${relative(projectRoot, path)}:${lineNumberAt(source, match.index ?? 0)} static template button label: ${literal}`,
                ]
              : [];
          });
        });
