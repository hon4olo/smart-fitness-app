...
describe('localization pluralization', () => {
  it('selects English singular and plural forms', () => {
    expect(selectPluralForm('en', 0, forms)).toBe('{count} items');
  ...
  it('selects Russian one, few, and many forms', () => {
    ...
    expect(selectPluralForm('ru', 25, forms)).toBe('{count} items-many');
    expect(selectPluralForm('ru', 101, forms)).toBe('{count} item');
    expect(selectPluralForm('ru', 111, forms)).toBe('{count} items-many');
    expect(selectPluralForm('ru', -1, forms)).toBe('{count} item');
    expect(selectPluralForm('ru', -2, forms)).toBe('{count} items-few');
    expect(selectPluralForm('ru', -5, forms)).toBe('{count} items-many');
    expect(selectPluralForm('ru', 1.5, forms)).toBe('{count} items');
    expect(selectPluralForm('ru', Number.NaN, forms)).toBe('{count} items');
    expect(selectPluralForm('ru', Number.POSITIVE_INFINITY, forms)).toBe('{count} items');
  });
