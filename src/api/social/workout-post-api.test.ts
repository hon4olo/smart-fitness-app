...
describe("social workout post API", () => {
  ...
  it("encodes profile pagination without exposing cursor structure", async () => {
    ...
    expect(request).toHaveBeenCalledWith({
      method: "GET",
      path: "/v1/social/profiles/name%2Fsegment/workout-posts?limit=20&cursor=opaque%2B%2Fcursor",
      headers: { authorization: "Bearer access-token" },
      retry: false,
    });
  });
  ...
  it("rejects invalid local page bounds before sending a request", async () => {
    ...
    await expect(
      api.listWorkoutPosts("coach_ivan", { limit: 51 }),
    ).rejects.toThrow("Social workout post limit must be between 1 and 50");
