...
const toDomainExercise = (row: ExerciseFixtureRow): Exercise => {
  ...
  const secondaryMuscles = uniqueStrings(row.secondaryMuscles ?? []);
  const equipment = uniqueStrings(row.equipment ? [row.equipment] : []);
  const aliases = uniqueStrings(row.aliases ?? []);

  return createExercise({
    id: row.internalId ?? row.id ?? normalize(row.name).replace(/\s+/g, '-'),
    name: row.name,
    aliases,
    ...
    secondaryMuscles,
    equipment,
    instructions: uniqueStrings(row.instructions ?? []),
