...
const SYNC_STATES = ['idle', 'syncing', 'success', 'error', 'offline', 'conflict'] as const;
type SyncState = (typeof SYNC_STATES)[number];

const isSyncState = (value: string): value is SyncState =>
  SYNC_STATES.some((state) => state === value);
