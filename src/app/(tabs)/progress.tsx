...
const formatWorkoutMonthDay = (dateString: string) => {
  const date = new Date(dateString);
...
export default function ProgressScreen() {
  ...
  const getExerciseTrendLabel = (entry: (typeof exerciseTrendData)[number]) => {
    ...
    return exerciseTrendDateCounts[dateKey] > 1
      ? formatWorkoutTime(entry.finishedAt)
      : formatWorkoutDate(entry.finishedAt);
  };
  ...
  const getWorkoutVolumeLabel = (entry: (typeof workoutVolumeData)[number]) => {
    ...
    return workoutVolumeDateCounts[dateKey] > 1
      ? formatWorkoutTime(entry.finishedAt)
      : formatWorkoutMonthDay(entry.finishedAt);
  };
  ...
  const formatTrendDelta = (delta: number) => `${delta >= 0 ? '+' : ''}${delta.toFixed(1)} kg`;
  const formatExerciseTrendDelta = (delta: number) => `${delta >= 0 ? '+' : ''}${delta.toFixed(1)} kg`;
  ...
  return (
    <ScrollView
      ...
      <View style={styles.container}>
        ...
        <AppCard>
          <Pressable
            ...
            <Text style={styles.sectionTitle}>
              {getSectionTitle('Body Weight Trend', isBodyWeightTrendExpanded)}
            </Text>
          </Pressable>
        ...
        <AppCard>
          <Pressable
            onPress={() => setIsWorkoutVolumeExpanded((current) => !current)}
            style={styles.collapsibleHeader}>
            <Text style={styles.sectionTitle}>
              {getSectionTitle('Workout Volume Trend', isWorkoutVolumeExpanded)}
            </Text>
          </Pressable>

          {isWorkoutVolumeExpanded ? (
            <>
              <TrendChart
        ...
        <AppCard>
          ...
          <AppButton
            disabled={isMeasurementDisabled}
                    ...
                    <View style={styles.exerciseHistory}>
                      {exerciseHistory.slice(0, 10).map((set, index) => (
                        <View
                          ...
                          <View style={styles.rowText}>
                            <Text style={styles.label}>{formatWorkoutDate(set.finishedAt)}</Text>
                            <Text style={styles.value}>
