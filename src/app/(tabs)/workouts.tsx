...
export default function WorkoutsScreen() {
  ...
  const workoutsInsight = (() => {
    ...
    return {
      ...
      detail: `${latestCompletedSession?.workoutTitle ?? 'Your last workout'} finished on ${formatShortDateTime(latestCompletedSession?.finishedAt ?? '')} with ${getSessionVolume(latestCompletedSession ?? completedSessions[0]).toLocaleString()} kg volume.`,
