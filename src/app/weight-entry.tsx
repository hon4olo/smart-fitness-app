...
export default function WeightEntryScreen() {
  ...
  const saveWeight = () => {
    ...
    const now = new Date();
    addWeightEntry({ id: createUuid(), date: formatShortDate(now.toISOString()), weight: parsedWeightKg, createdAt: now.toISOString() });
    setError('');
    setWeight('');
    router.back();
  };
...
const styles = StyleSheet.create({
  ...
  error: { color: Colors.dark.error, fontSize: 13, lineHeight: 18, marginTop: Spacing.one },
